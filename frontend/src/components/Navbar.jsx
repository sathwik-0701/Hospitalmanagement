  ...
  return (
    <div className="flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-400 bg-white sticky top-0 z-50">
      ...
      <div className="flex items-center gap-4">
        ...
        />
        <div
          className={`fixed md:hidden right-0 top-0 z-20 overflow-hidden bg-white transition-all duration-300 ${
          ...
          <ul className="flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium">
            {navLinks.map((nav, index) => (
              <NavLink
                ...
                to={nav.path}
              >
                <p className="px-4 py-2 rounded inline-block text-gray-800 hover:text-primary transition-colors">
                  {nav.title}
                </p>
              </NavLink>
            ))}
